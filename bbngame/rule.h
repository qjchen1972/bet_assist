...
public:
	...
	int getBetMoney(UserStat *user, GameData *data, uint8_t &betType, int &b1, int &b2, int &b3, int8_t &betId) {
		...
		return betMoney(data, *colRoad, user->tempProcer, *gameProcer, gConfig.avgLowIsFake,
	...
	float getAvg(std::vector<Kline_t> data, int size, int pos=0) {
		float total = 0;
		int num = 0;
		int len = data.size() - 1 - pos;	
		for (int i = len; i >= 0; i--) {
			total += data[i].end;
			num++;
			if (num == size) break;
	...
	int moneyRate(int pos, bool avgRate, int money) {

		if(avgRate && pos == 2) return int(money * gConfig.moneyRate / 2);
		return int(money * gConfig.moneyRate);		
	}

	

	int betMoney(GameData *data,
		            std::vector<std::vector<Round_t>> &colRoad, 
		            StatProc &allProcer, 
		            StatProc &gameProcer,
		            bool useAvg,
		...
		int total = 0;
		/*if (data->roomId <= 5) {
			for (int i = 0; i < data->showedCard.size(); i++) {
				if (i == 8 || i == 7 || i == 6 || i == 5) {
					data->showedCard[i] += 24;
					if (data->showedCard[i] > 32) data->showedCard[i] = 32;
				}
			}
		}
		else if (data->roomId <= 11) {
			for (int i = 0; i < data->showedCard.size(); i++) {
				if (i == 7 || i == 6 || i == 5 || i == 4) {
					data->showedCard[i] += 24;
					if (data->showedCard[i] > 32) data->showedCard[i] = 32;
		...
		*/
		...
		Baccarat bjl;
		total = bjl.cardNum(data->showedCard);
		if (data->midIn || total < 200) return 0;
		
		bjl.setLeftCard(data->showedCard);
		int player, banker;
		...
		//if (player > banker) action = "p";
		//else if (player < banker) action = "b";
		//else return 0;

		int pos = gamePos(allProcer);
		if (upNum >= 3) money = money * 2;

		betType = getGameMode(pos, useAvg);
		//betType = REALBET_TYPE;
		//int money = moneyRate(betRule.getBetMoney(betType, gameProcer, betId) * gConfig.moneyRate);
		//money = moneyRate(pos, gConfig.avgRate, betRule.getBetMoney(betType, gameProcer, betId));
		//if (money == 0) return 0;
		/*if (money < 10) {
			betType = FAKEBET_TYPE;
			//money = 10;
		}*/
	...
	int betMoney(std::vector<std::vector<Round_t>> &colRoad,
		...
		int pos = gamePos(allProcer);
		betType = getGameMode(pos, useAvg);
		//int money = moneyRateint(betRule.getBetMoney(betType, gameProcer, betId) * gConfig.moneyRate);
		int money = moneyRate(pos, gConfig.avgRate, betRule.getBetMoney(betType, gameProcer, betId));
		//if (money == 0) return 0;
		/*if (money < 10) {
			betType = FAKEBET_TYPE;
			//money = 10;
	...
	int avgNum(std::vector<Kline_t> klineSet) {
		float avg5 = getAvg(klineSet, gConfig.avgNum);
		if (klineSet.size() == 0)  return 0;
		if (klineSet[klineSet.size() - 1].end < avg5) return 0;
		if (klineSet.size() <= 1) return 1;

		avg5 = getAvg(klineSet, gConfig.avgNum, 1);
		if (klineSet[klineSet.size() - 2].end < avg5) return 1;
		if (klineSet.size() <= 2) return 2;

		avg5 = getAvg(klineSet, gConfig.avgNum, 2);
		if (klineSet[klineSet.size() - 3].end < avg5) return 2;

		return 3;
	}

	int gamePos(StatProc &procer) {
		std::vector<Kline_t> klineSet;
		procer.getKline(klineSet, gConfig.avgNum * 2);
		if (klineSet.size() <= 1) {
			upNum = 0;
			return 0;
		}
		float avg5 = getAvg(klineSet, gConfig.avgNum);
		float avg10 = getAvg(klineSet, gConfig.avgNum * 2);
		upNum = avgNum(klineSet);

		if (klineSet[klineSet.size() - 1].end > klineSet[klineSet.size() - 2].end) {

			if (klineSet[klineSet.size() - 1].end > avg5 && klineSet[klineSet.size() - 1].end > avg10)
				return 1;
			else if (klineSet[klineSet.size() - 1].end >= avg5)
				return 2;
			else
				return 3;
		}
	...
	uint8_t getGameMode(int pos /*StatProc &procer*/, bool useAvg) {
		//if (!gConfig.avgLowIsFake) return REALBET_TYPE;

		if(!useAvg) return REALBET_TYPE;
		if (pos <= 1) return REALBET_TYPE;		
		else if (pos == 2) {
			if(rand() % 10 >= 4 ) return REALBET_TYPE;
			else return FAKEBET_TYPE;
		}
		else if (pos == 3) {
			if (rand() % 10 >= 6) return REALBET_TYPE;
			else return FAKEBET_TYPE;
		}
		else return FAKEBET_TYPE;
		return FAKEBET_TYPE;
