...
public:
	...
	std::string getDay(uint64_t time) {
		time_t now = time;
		char stTmp[64];
		struct tm tmTmp;

		localtime_s(&tmTmp, &now);
		sprintf_s(stTmp, "%d-%02d-%02d",
			1900 + tmTmp.tm_year,
			tmTmp.tm_mon + 1,
			tmTmp.tm_mday);
...
public:
	...
	void setProfit(Round_t &data, bool needKline=false, bool tempStart=false) {
		...
		if (data.type == REALBET_TYPE && !data.isSmallBet)
	...
	void getKline(std::vector<Kline_t> &set, int size) {
		std::lock_guard<std::mutex> guard(mtx);
		set.clear();
		if (kLineSet.size() == 0) return;
		if (kLineSet.size() < size) size = kLineSet.size();
		int pos = kLineSet.size() - size;		
		set.insert(set.begin(), kLineSet.end() - size, kLineSet.end());
		//set = kLineSet;
	}
...
public:
	GameData() {
		showedCard = std::vector<int>(10, 0);
	}
	~GameData() {}	
	...
	void setInitRoad(std::string strRoad, std::string roundNo="") {
		...
		if (gameNum != temp) {
			...
			showedCard = std::vector<int>(10, 0);
			Tool tool;
			std::string day = tool.getDay(time(NULL));
			char buf[512];
			curDataPath = "data/";
			sprintf_s(buf, "%s_%d_%d.txt", day.c_str(), roomId, gameNum);
			curDataPath += buf;			
		}
		else allRoad[allRoad.size() - 1] = road;

		if (road.size() == 0) midIn = false;
		else	midIn = true;
		gameNum = temp;
	...
	int  newGame(std::string roundNo) {
		std::lock_guard<std::mutex> guard(mtx);
		if (getGame(roundNo) == gameNum) return 0;
	...
	void overRound() {
		...
		//writeData();
		updateShowedCard(cur_round.cardStr, showedCard);
		roundIsOver = true;
		rateStr = "";
	}
	...
	void writeData() {
		std::ofstream f;
		f.open(curDataPath, std::ios::out | std::ios::app);
		if (!f.is_open())  return;
		f << cur_round.cardStr<< " " << (int)cur_round.cardAns << std::endl;
		f.close();
	}
...
private:	
	...
	std::string curDataPath = "data/";

	std::mutex mtx;
	std::vector<std::vector<Round_t>>  allRoad;
	

	void updateShowedCard(std::string strCard, std::vector<int> &showedCard) {

		Tool tool;
		std::vector<std::string> allans;
		tool.my_split(strCard, ',', allans);

		for (int m = 0; m < allans.size(); m++) {
			if (allans[m].size() == 0) continue;
			std::string sub = allans[m].substr(2, allans[m].size() - 2);
